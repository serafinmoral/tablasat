...
#            print("resuelto devuelvo ",asignaciones)
    ...
    variable = obtenerVariable3c(formula, N1, N2)
    if formula.contradict:
        tapren.insertar(frozenset(formula.apren))
...
def asignagreedy(formula):
    ...
    while lista:
        v = max(lista, key=lambda x: abs(
            len(formula.indices.get(x, set()))-len(formula.indices.get(-x, set()))))
...
#        print("fin ", vmax, len(apren))
    if frozenset({vmax}) in r.listaclaus and frozenset({-vmax}) in r.listaclaus:
        ...
        formula.contradict = True
        formula.solved = True
        formula.apren = r.refer.get(frozenset({vmax}), set()).union(
            r.refer.get(frozenset({-vmax}), set()))
        if (not formula.apren):
            print("vamx ", vmax, r.refer.get(
                frozenset({vmax}), set()), r.refer.get(frozenset({-vmax}), set()))
        return vmax
